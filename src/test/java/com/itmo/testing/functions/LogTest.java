...
class LogTest {
    ...
    void setUp() {
        ...
        when(ln.calculate(3)).thenReturn(Math.log(3));
    ...
    void log2Test(double value) {
        when(ln.calculate(value)).thenReturn(Math.log(value));
        double expected = new BigDecimal(Double.toString(Math.log(value)/Math.log(2)))
                .setScale(4, RoundingMode.HALF_UP)
                .doubleValue();
        double actual = new BigDecimal(Double.toString(log2.calculate(value)))
                .setScale(4, RoundingMode.HALF_UP)
                .doubleValue();
    ...
    @ParameterizedTest
    @ValueSource(doubles = {0.3, 1, 100, 22, 7.04, 33.333})
    void log3Test(double value) {
        when(ln.calculate(value)).thenReturn(Math.log(value));
        double expected = new BigDecimal(Double.toString(Math.log(value)/Math.log(3)))
                .setScale(4, RoundingMode.HALF_UP)
                .doubleValue();
        double actual = new BigDecimal(Double.toString(log3.calculate(value)))
                .setScale(4, RoundingMode.HALF_UP)
                .doubleValue();
        assertEquals(expected, actual);
    }
